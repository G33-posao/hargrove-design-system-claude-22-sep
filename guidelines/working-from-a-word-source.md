...
## PDF comments/annotations are part of the source — always read and apply them
A source PDF can carry review comments/annotations (sticky notes, highlights with text) alongside the page text — client corrections often live ONLY there (e.g. "Actual" casing fix, a new phone number). Extracting page text alone silently drops them. On every source PDF, extract the annotations too (pdf.js: `page.getAnnotations()` — read `contents`, `title`/author, and the page they sit on) and apply each one, or flag it in chat if it conflicts with another rule. A comment is client-approved correction, so it outranks the verbatim rule the same way the mzoubi address fix does. (Failure case: Aug 2026 — Andreina's PDF comments were missed on first pass because only page text was read.)

## Source emphasis maps 1:1 — never relocate it
Whatever the source itself sets apart — a framed/outlined note, a shaded box, a callout, bolded lines — **stays the emphasized element** in the redesign, with equivalent visual weight (a box stays a box). And the reverse: an element the source did NOT set apart (an ordinary bullet, a plain list) never gets promoted into the piece's featured card while the source's own emphasized element is demoted to plain text. Swapping emphasis between elements rewrites the author's hierarchy just as surely as rewriting the words. (Failure case: Trust Funding deck p. 3 — the source framed the "Not all of your assets…" note; the redesign unframed it and boxed the examples list instead.)
...
Two refinements from the WEG case study (`case-study-lessons-weg.md` A2, A3, A5): **one
topic = one text frame** — consecutive paragraphs of a section are one block with
paragraph breaks, never a stack of separate boxes; **a source list is reproduced in full
and uniformly** — N bullets become N identical cards (2×2 for four), never three cards
plus a demoted sentence; and bold *runs* inside body survive 1:1 (`internal survey`
stays bold, nothing else gains bold).
...
across the full content width, two lines, tracking 0 (never negative) — and run the
